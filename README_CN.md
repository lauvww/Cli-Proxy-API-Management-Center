...
- **号池管理** 已经成为独立的一组管理能力。配置页保留号池模式开关，而专门的“号池管理”页面只会在后端实际开启 auth-pool 模式后显示。
...
- 配置页保留号池模式开关；Auth Pool 页面本身不再显示该开关。
- 运行时仍采用稳定的“单当前号池”语义：可以维护多个号池路径，但同一时刻只有一个当前号池真正参与服务。
- Auth Pool 页面支持切换当前号池、修改当前号池轮询策略，并直接搜索当前号池中的认证文件以及快速启用/停用这些文件。
- Usage 页面在号池模式开启时会默认展示当前号池的统计，并优先显示后端返回的范围提示。
- Auth Files 页面会以当前号池为作用域，上传、删除、编辑和启停操作都只作用于当前号池。
- 构建产物会注入本地管理页标记，桌面发布时可优先保留本地生成的 `management.html`。
...
- 桌面版发布时通常会把构建产物 `dist/index.html` 复制或重命名为 `management.html`。
