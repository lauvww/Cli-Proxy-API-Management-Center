...
const normalizeAuthPoolConfig = (payload: unknown) => {
  if (!isRecord(payload)) return undefined;
  const source = isRecord(payload['auth-pool'])
    ? (payload['auth-pool'] as Record<string, unknown>)
    : payload;
  if (!isRecord(source)) return undefined;

  const rawPaths = Array.isArray(source.paths) ? source.paths : [];
  const paths = Array.from(
    new Set(rawPaths.map((item) => String(item ?? '').trim()).filter(Boolean))
  );

  const activePathRaw = source['active-path'] ?? source.activePath;
  const activePathAlt = source['active_path'];
  const activePath =
    typeof activePathRaw === 'string' && activePathRaw.trim() ? activePathRaw.trim() : undefined;
  const resolvedActivePath =
    activePath ??
    (typeof activePathAlt === 'string' && activePathAlt.trim() ? activePathAlt.trim() : undefined);
  const routingValue =
    source['routing-strategy-by-path'] ??
    source['routing_strategy_by_path'] ??
    source.routingStrategyByPath;
  const routingSource = isRecord(routingValue) ? (routingValue as Record<string, unknown>) : null;
  const routingStrategyByPath = routingSource
    ? Object.fromEntries(
        Object.entries(routingSource)
          .map(([path, strategy]) => [String(path ?? '').trim(), String(strategy ?? '').trim()])
          .filter(([path, strategy]) => path && strategy)
      )
    : undefined;
